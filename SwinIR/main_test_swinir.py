...
def main(task, scale, noise,jpeg,training_path_size,large_model,model_path,folder_lq,folder_gt,tile,tile_overlap):

    
    class Parameters:
        def __init__(self, task, scale, noise,jpeg,training_path_size,large_model,model_path,folder_lq,folder_gt,tile,tile_overlap):
            self.task = task
            self.scale = scale
            self.noise = noise
            self.jpeg = jpeg
            self.training_path_size=training_path_size
    ...
    #args=Parser('real_sr',4,15,40,128,True,'experiments/pretrained_models/003_realSR_BSRGAN_DFOWMFC_s64w8_SwinIR-L_x4_GAN.pth','BSRGAN/testsets/RealSRSet',None,None,32)  
    args= Parameters(task, scale, noise,jpeg,training_path_size,large_model,model_path,folder_lq,folder_gt,tile,tile_overlap)
